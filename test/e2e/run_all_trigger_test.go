...
// TestE2E_RunAllOnTrigger validates the complete gorts workflow:
// gorts tests -> gorts baseline -> gorts mapping -> gorts select
//
// This particular test confirms the usage of run all trigger
// that should select all tests if file triggering the run-all
// is modified
func TestE2E_RunAllOnTrigger(t *testing.T) {
	...
	manifestFile := filepath.Join(outputDir(t, "run_all_trigger"), "manifest.json")
	...
	coverageDir := filepath.Join(outputDir(t, "run_all_trigger"), "coverage_test")
	baselineFile := filepath.Join(outputDir(t, "run_all_trigger"), "baseline_test.json")
	...
	mappingFile := filepath.Join(outputDir(t, "run_all_trigger"), "mapping_file_level.json")
	...
	// Phase 4: gorts select (with checked out newer commit) with run-all-on param
	// In the currentCommit these files are changed: internal/model/book.go and
	// internal/model/book_test.go, so having any of these in the --run-all-on
	// should result in selecting all available tests
	...
	selectionFile := filepath.Join(outputDir(t, "run_all_trigger"), "selection_newer_commit.json")
	...
	stdout, stderr, exitCode = runGortsInDir(t, testRepoPath,
		...
		"--repo", testRepoPath,
		"--run-all-on", "internal/model/book_test.go",
		"--strip-prefix", "",
	)
	if exitCode != 0 {
	...
	if selection.Stats.SelectedTests != selection.Stats.TotalTests {
		t.Fatal("modified file included as param with '--run-all-on' should result in all tests being selected", err)
	}
}
